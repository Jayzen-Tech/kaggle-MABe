...
"""F Beta customized for the data format of the MABe challenge.
   Ensemble: XGBoost + LightGBM + CatBoost (simple average of probabilities)
"""
...
# Debug helpers
# ------------------------
DEBUG_MODE = os.environ.get("MABE_DEBUG", "1") not in {"0", "false", "False"}
DEBUG_LOG_PATH = os.environ.get("MABE_DEBUG_LOG", "/kaggle-MABe/debug.log")
        ...
        if not _DEBUG_LOG_INITIALIZED:
            try:
                with open(DEBUG_LOG_PATH, "w", encoding="utf-8") as fh:
                    fh.write("[DEBUG] Log initialized\n")
            except OSError as exc:
                print(f"[DEBUG] Failed to initialize log file {DEBUG_LOG_PATH}: {exc}")
            _DEBUG_LOG_INITIALIZED = True
        try:
            with open(DEBUG_LOG_PATH, "a", encoding="utf-8") as fh:
                fh.write(f"{msg}\n")
        except OSError as exc:
            print(f"[DEBUG] Failed to write log file {DEBUG_LOG_PATH}: {exc}")
        print(f"[DEBUG] {msg}")
...
# ------------------------
class CFG:
    root_dir = "/kaggle-MABe"
    dataset_dir = f"{root_dir}/dataset/MABe-mouse-behavior-detection"

    train_path = f"{dataset_dir}/train.csv"
    test_path = f"{dataset_dir}/test.csv"
    train_annotation_path = f"{dataset_dir}/train_annotation"
    train_tracking_path = f"{dataset_dir}/train_tracking"
    test_tracking_path = f"{dataset_dir}/test_tracking"

    model_path = f"{root_dir}/models"
    model_name = "ensemble_xgb_lgb_cat"

    # Default mode gets overridden per notebook
    mode = "validate"

    n_splits = 3
    cv = StratifiedGroupKFold(n_splits)

    # keep an XGB default here for backwards compat; actual models instantiated per-action below
    model = XGBClassifier(
        verbosity=0,
        random_state=42,
        n_estimators=250,
...
os.makedirs(os.path.join(CFG.model_path, CFG.model_name), exist_ok=True)


# ------------------------
# Data loading + helper generator
# ------------------------
...
def generate_mouse_data(dataset, traintest, traintest_directory=None, generate_single=True, generate_pair=True):
    if traintest_directory is None:
        traintest_directory = f"/kaggle-MABe/dataset/MABe-mouse-behavior-detection/{traintest}_tracking"
    ...
    for _, row in dataset.iterrows():
        ...
        if traintest == 'train':
            try:
                ...
                debug(f"[generate_mouse_data] Missing annotation parquet for {lab_id}/{video_id}")
                continue
...
# ------------------------
# Training / predict helpers (关键修改在这里)
# ------------------------
...
# ------------------------
# cross_validate_classifier: train three base learners per action -> average OOF
# ------------------------
def cross_validate_classifier(X, label, meta, body_parts_tracked_str, section):
    ...
    base_section_dir = os.path.join(CFG.model_path, CFG.model_name, str(section))
    os.makedirs(base_section_dir, exist_ok=True)
    ...
    learner_params = {name: model.get_params() for name, model in base_learners.items()}
    debug(f"[section {section}] body_parts={body_parts_tracked_str} base_learners={learner_params}")
    ...
    for action in label.columns:
        ...
        debug(f"[section {section}] action={action} samples={X_action.shape[0]}, positives={int(y_action.sum())}, negatives={int((y_action == 0).sum())}, groups={len(np.unique(groups_action))}")
        ...
        if not (y_action == 0).all():
            try:
                with warnings.catch_warnings():
                    ...
                    save_dir = os.path.join(base_section_dir, action)
                    os.makedirs(save_dir, exist_ok=True)

                    # 为每个基学习器训练并收集 OOF
                    oof_preds_models = []
                    for model_name, model_obj in base_learners.items():
                        save_dir_model = os.path.join(save_dir, model_name)
                        os.makedirs(save_dir_model, exist_ok=True)

                        debug(f"[section {section}] action={action} training model={model_name} params={model_obj.get_params()}")
                        trainer = Trainer(
                            ...
                            verbose=False,
                            save=True,
                            save_path=save_dir_model
                        )

                        trainer.fit(X_action, y_action)
                        # 假设 trainer.oof_preds 是概率形式
                        oof_action_model = trainer.oof_preds
                        oof_preds_models.append(oof_action_model)

                        # 保存每个基学习器的 oof
                        joblib.dump(oof_action_model, os.path.join(save_dir_model, "oof_pred_probs.pkl"))
                        debug(f"[section {section}] action={action} model={model_name} oof_shape={np.shape(oof_action_model)}")

                        del trainer
                        gc.collect()

                    # 平均 OOF -> ensemble OOF
                    oof_action = np.mean(oof_preds_models, axis=0)
                    debug(f"[section {section}] action={action} ensemble_oof_stats min={float(np.min(oof_action)):.4f} max={float(np.max(oof_action)):.4f} mean={float(np.mean(oof_action)):.4f}")
                    ...
                    debug(f"[section {section}] action={action} tuned_threshold={threshold:.4f} binary_f1={f1:.4f}")
                ...
                debug(f"[section {section}] action={action} exception={e}")
                oof_action = np.zeros(len(y_action))
                print(f"    Exception for action {action}: {e}")
                print(f"    F1: 0.0000 (0.00) Section: {section} Action: {action}")

        else:
            oof_action = np.zeros(len(y_action))
            print(f"    No positive labels. F1: 0.0000 (0.00) Section: {section} Action: {action}")
            debug(f"[section {section}] action={action} skipped due to no positives.")

        oof_column = np.zeros(len(label))
        oof_column[action_mask] = oof_action
        oof[action] = oof_column
        debug(f"[section {section}] action={action} stored_oof_nonzero={(oof_column > 0).sum()}")
    ...
    for switch_te, data_te, meta_te, actions_te in generator:
        ...
        try:
            ...
            for action in actions_te:
                action_dir = os.path.join(CFG.model_path, CFG.model_name, str(section), action)

                # 查找 action 下所有模型（子目录里 Trainer 保存的 pkl），并对每个 trainer 做预测取平均
                files = glob.glob(os.path.join(action_dir, "*", "*_trainer_*.pkl"))
                model_preds = []
                for fpath in files:
                    try:
                        trainer = joblib.load(fpath)
                        # 假设 trainer.predict 返回概率；如果返回类别则需改为 predict_proba
                        model_pred = trainer.predict(X_te)
...
if CFG.mode == "validate":
    ...
    thresholds_path = os.path.join(CFG.model_path, CFG.model_name, "thresholds.pkl")
    if not os.path.exists(thresholds_path):
        raise FileNotFoundError(
            f"Missing thresholds at {thresholds_path}. Run the validation notebook before submit mode."
        )
...
for section in range(1, len(body_parts_tracked_list)):
    ...
    try:
        ...
        subset_video_count = train_subset['video_id'].nunique()
        subset_lab_count = train_subset['lab_id'].nunique()
        debug(f"[section {section}] train_subset rows={len(train_subset)}, videos={subset_video_count}, labs={subset_lab_count}")
        ...
        debug(f"[section {section}] generated samples: single={gen_counts.get('single', 0)}, pair={gen_counts.get('pair', 0)}")
        ...
        if len(single_mouse_list) > 0:
            ...
            debug(f"[section {section}] single features shape={X_tr.shape}, labels shape={single_mouse_label.shape}, meta rows={len(single_mouse_meta)}")
            pos_counts = single_mouse_label.sum().sort_values(ascending=False)
            pos_info = pos_counts[pos_counts > 0].head(10).to_dict()
            debug(f"[section {section}] single positive label counts (top10)={pos_info}")
            zero_cols = (pos_counts == 0).sum()
            debug(f"[section {section}] single zero-positive columns={zero_cols}")
            ...
            debug(f"[section {section}] no single samples generated.")
        ...
        if len(mouse_pair_list) > 0:
            ...
            debug(f"[section {section}] pair features shape={X_tr.shape}, labels shape={mouse_pair_label.shape}, meta rows={len(mouse_pair_meta)}")
            pair_pos_counts = mouse_pair_label.sum().sort_values(ascending=False)
            pair_pos_info = pair_pos_counts[pair_pos_counts > 0].head(10).to_dict()
            debug(f"[section {section}] pair positive label counts (top10)={pair_pos_info}")
            pair_zero_cols = (pair_pos_counts == 0).sum()
            debug(f"[section {section}] pair zero-positive columns={pair_zero_cols}")
            ...
            debug(f"[section {section}] no pair samples generated.")
...
if CFG.mode == 'validate':
    ...
    joblib.dump(f1_df, os.path.join(model_dir, 'scores.pkl'))
    debug(f"[post] thresholds saved to {os.path.join(model_dir, 'thresholds.pkl')} with keys single={sorted(thresholds.get('single', {}).keys())} pair={sorted(thresholds.get('pair', {}).keys())}")
    debug(f"[post] scores saved to {os.path.join(model_dir, 'scores.pkl')} rows={len(f1_df)} mean_f1={f1_df['binary F1 score'].mean():.4f}")
...
if CFG.mode == 'submit':
    ...
    submission_robust.to_csv('/kaggle-MABe/submission/submission.csv', index=True)
