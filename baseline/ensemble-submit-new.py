...
"""Submit-time ensemble inference for the MABe challenge.
   Ensemble: XGBoost + CatBoost (simple average of probabilities)
"""
...
class CFG:
    ...
    # Force submit mode for this script
    mode = "submit"

    n_splits = 3
    cv = StratifiedGroupKFold(n_splits) if StratifiedGroupKFold is not None else None
...
def robustify(submission, dataset, traintest, traintest_directory=None):
    ...
    submission[['start_frame', 'stop_frame']] = submission[['start_frame', 'stop_frame']].astype(np.int64)
...
def tune_threshold(oof_action, y_action):
    if optuna is None:
        raise RuntimeError("Optuna is required for threshold tuning.")
    def objective(trial):
...
for section in range(1, len(body_parts_tracked_list)):
    ...
    try:
        ...
        single_thresholds = thresholds.get("single", {}).get(f"{section}")
        pair_thresholds = thresholds.get("pair", {}).get(f"{section}")

        if CFG.mode == 'submit':
            single_thresholds_to_use = single_thresholds if single_thresholds is not None else {}
            if single_thresholds is None:
                print("    No single thresholds found; using default fallback.")
            temp_submission_list = submit(body_parts_tracked_str, 'single', section, single_thresholds_to_use)
            submission_list.extend(temp_submission_list)
            del temp_submission_list
            gc.collect()

            pair_thresholds_to_use = pair_thresholds if pair_thresholds is not None else {}
            if pair_thresholds is None:
                print("    No pair thresholds found; using default fallback.")
            temp_submission_list = submit(body_parts_tracked_str, 'pair', section, pair_thresholds_to_use)
            submission_list.extend(temp_submission_list)
            del temp_submission_list
            gc.collect()
...
if CFG.mode == 'submit':
    ...
    submission_robust.to_csv('/kaggle-MABe/submission/submission-ensemble.csv', index=True)
