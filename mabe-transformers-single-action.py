...
def _safe_hypot(dx: pd.Series, dy: pd.Series) -> pd.Series:
    """Compute sqrt(dx^2 + dy^2) without triggering invalid sqrt warnings."""
    mask = dx.notna() & dy.notna()
    dist_sq = (dx.pow(2) + dy.pow(2)).where(mask, 0.0)
    dist = np.sqrt(dist_sq)
    return dist.where(mask, np.nan)
